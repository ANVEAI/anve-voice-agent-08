...
const VAPI_CONFIG = {
  publicKey: "ae97b042-d2fc-469e-ba4e-89dad3a2d28a",
  assistantId: "0f46e5a1-ddb6-49b4-a6ca-f342e3d82a00",
};
  ...
  const findInputField = (targetText?: string, fieldType?: string): HTMLElement | null => {
    ...
    if (targetText) {
      const label = Array.from(document.querySelectorAll("label")).find((l) =>
        l.textContent?.toLowerCase().includes(targetText.toLowerCase()),
      );
      if (label) {
    ...
    if (fieldType) {
      ...
      const nameElement = document.querySelector(
        `input[name="${fieldTypeLower}"], input[name*="${fieldTypeLower}"]`,
      ) as HTMLElement;
